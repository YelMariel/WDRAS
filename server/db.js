...
app.post('/calculateAndStoreDifference', (req, res) => {
  // Assuming you have a valid connection pool named 'pool'
  pool.query('SELECT startingrefills, total_value, (startingrefills - total_value) as remaining_refills FROM logs_consumed', (queryErr, queryResults) => {
    if (queryErr) {
      console.error('Error querying the database:', queryErr);
      res.status(500).json({ error: 'Error querying the database.' });
    } else {
      // Assuming there is only one row in the result set
      const remaining_refills = queryResults[0].remaining_refills;

      // Now, you can insert the remaining_refills into the database
      pool.query('INSERT INTO logs_consumed (remaining_refills) VALUES (?)', [remaining_refills], (insertErr, insertResults) => {
        if (insertErr) {
          console.error('Error storing difference in the database:', insertErr);
          res.status(500).json({ error: 'Error storing difference in the database.' });
        } else {
          res.json({ success: true, remaining_refills });
        }
      });
    }
  });
...
app.post('/AddContainer', (req, res) => {
  ...
  pool.query(
    'INSERT INTO logs_consumed (total_value, startingrefills, remaining_refills) VALUES (?, ?, ?)',
    [totalValue, startingRefills, remainingRefills],
    (insertErr, insertResults) => {
      if (insertErr) {
        console.error('Error inserting record:', insertErr);
        res.status(500).json({ error: 'Error inserting record.' });
      } else {
        res.json({ success: true, remainingRefills });
      }
    }
  );
});
